...
main (int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (strcmp(argv[1],"--help")==0) {
			...
			std::cout << "If --header is supplied, a 32-bit sequence number and 32-bit data size field are sent as a packet header." << std::endl;
			std::cout << "If --header-crc is supplied, the header is sent along with a trailing 32-bit CRC." << std::endl;
			std::cout << std::endl;
			exit(0);
		}
